/*
 Yeni App — FONET Gebelikte Apandisit Hasta Tarayıcı v1.0.0
...
(() => {
  ...
  async function patientData(p){
    if(!p.gelisId||!p.birimSevkId)throw Error('Hasta geliş ID veya birim sevk ID bulunamadı; FONET açık listesinden tarayın.');
    ...
    const [panel,consult,services,stay,rad,lab,pathology,orders,history]=await Promise.all([
      ...
      attempt([`/Order/Order/getHastaGelisOrderList/${p.gelisId}`,`/Klinik/Order/getHastaGelisOrderList/${p.gelisId}`]),
  ...
  async function run(){state.running=true;state.stop=false;state.paused=false;$('#fga-start').disabled=true;$('#fga-pause').disabled=false;$('#fga-stop').disabled=false;for(const p of state.patients){if(state.stop)break;while(state.paused&&!state.stop)await sleep(250);try{log(`${p.operationNo||p.tc} taranıyor...`);state.results.push(makeRow(p,await patientData(p)));}catch(e){state.errors++;const r=Object.fromEntries(headers.map(h=>[h,NA]));r['İşlem No']=p.operationNo||NA;r.TC=p.tc||NA;r['Tarama durumu']=`Hata: ${e.message}`;state.results.push(r);}state.done++;render();status();}state.running=false;$('#fga-pause').disabled=true;$('#fga-stop').disabled=true;$('#fga-csv').disabled=!state.results.length;status(state.stop?'Tarama durduruldu. Mevcut sonuçlar indirilebilir.':'Tarama tamamlandı. Sonuçları CSV olarak indirin.');}

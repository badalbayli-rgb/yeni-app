...
(() => {
  ...
  function triggerExtButton(doc,el){
    const Ext=doc.defaultView.Ext,id=String(el?.id||'').replace(/-(?:btnEl|btnInnerEl|btnIconEl)$/,'');
    let cmp=Ext?.getCmp?.(id)||Ext?.get?.(el)?.component;
    if(!cmp&&Ext?.ComponentQuery)cmp=Ext.ComponentQuery.query('button').find(x=>up(x.getText?.()||x.text)==='SORGULA');
    if(cmp?.fireHandler){cmp.fireHandler();return true;}
    if(cmp?.handler){cmp.handler.call(cmp.scope||cmp,cmp);return true;}
    fireClick(el);return false;
  }
  ...
  function searchControls(){for(const d of docs()){const buttons=[...d.querySelectorAll('button,a,[role="button"],.x-btn')].filter(visible),query=buttons.find(x=>up(x.innerText)==='SORGULA');if(!query)continue;const fields=[...d.querySelectorAll('.x-field,.x-form-item')];const byLabel=label=>fields.filter(x=>visible(x)&&new RegExp(`^\\s*${label}\\s*:`, 'i').test(norm(x.innerText))).map(x=>x.querySelector('input')).find(x=>x&&!x.disabled&&!x.readOnly);const tcInput=byLabel('Kimlik No'),unitInput=byLabel('Ameliyathane');if(tcInput)return {doc:d,query,tcInput,unitInput,byLabel};}return null;}
  function setFonetField(doc,input,value){if(!input)return;const id=String(input.id||'').replace(/-inputEl$/,'');const cmp=doc.defaultView.Ext?.getCmp?.(id);if(cmp?.setValue){cmp.setValue(value);cmp.fireEvent?.('change',cmp,value);return;}setInput(input,value instanceof Date?`${String(value.getDate()).padStart(2,'0')}.${String(value.getMonth()+1).padStart(2,'0')}.${value.getFullYear()}`:value);}
  function clearOptionalCombo(doc,input){if(!input)return;const id=String(input.id||'').replace(/-inputEl$/,'');const cmp=doc.defaultView.Ext?.getCmp?.(id);if(cmp?.clearValue){cmp.clearValue();cmp.fireEvent?.('change',cmp,null);}}
  ...
  const isAppendectomy=x=>/\b(?:LAPAROSKOPİK\s+)?APP?ENDEKTOM|\bAPP?ENDİSİT|\bAPANDİSİT/i.test(text(x.raw||x));
